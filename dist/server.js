...
import { setAdminState, getAdminState, clearAdminTempData } from "./services/StateService.js";
import { getSuperAdminDetails, generateAdminOptions, handleAdminResponse } from "./services/AdminService.js";
import { isDuplicatePendingTrainee, isDuplicateTrainee } from "./services/TraineeService.js";
dotenv.config();
...
expressApp.get("/list-phone-numbers", async (req, res) => {
    try {
...
// Helper functions
async function isDuplicateAttendance(phoneNumber, date, db) {
...
async function isDuplicateAttendanceRequest(phoneNumber, date, db) {
...
async function getActiveTrainees(page = 1, pageSize = 10, db) {
    try {
        ...
        const trainees = querySnapshot.docs.map((doc) => {
            const data = doc.data();
            return {
                name: data.name,
...
async function getCompletedTrainees(db) {
    try {
...
// Message handlers
async function handleHiCommand(phoneNumber, db) {
...
async function handleTraineeOptions(phoneNumber, db) {
    try {
...
async function handleAttendanceResponse(phoneNumber, message, db) {
    try {
...
async function handleSandboxJoin(phoneNumber, db) {
    try {
